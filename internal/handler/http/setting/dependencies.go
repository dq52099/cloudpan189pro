...
func (h *handler) ensureSettingService(ctx *httpcontext.Context, busErr httpcontext.BusinessError) bool {
	if isNilDependency(h.settingService) {
		ctx.Fail(busErr.WithError(errors.New("系统配置服务未初始化")))
